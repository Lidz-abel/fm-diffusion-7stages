...
from src.visual_dataset import get_visual_dataloader
...
def parse_args():
    ...
    parser.add_argument("--dataset", type=str, default="pets", choices=["pets", "imagefolder"])
    parser.add_argument("--data_dir", type=str, default=str(ROOT / "data" / "visual64"))
    ...
    parser.add_argument("--save_path", type=str, default=str(ROOT / "checkpoints" / "visual64_unet_edm.pt"))
    parser.add_argument("--log_path", type=str, default=str(ROOT / "results" / "visual64" / "training_log_edm.csv"))
    parser.add_argument("--config_path", type=str, default=str(ROOT / "results" / "visual64" / "config_used_edm.json"))
    parser.add_argument("--curve_path", type=str, default=str(ROOT / "figures" / "visual64" / "training_curve_edm.png"))
...
def save_training_curve(loss_history: list[float], save_path: str | Path) -> None:
    ...
    plt.title("Visual64 Conditional EDM U-Net Training")
    plt.grid(alpha=0.2)
    plt.tight_layout()
...
def main():
    ...
    dataloader, dataset_info = get_visual_dataloader(
    ...
    null_label = dataset_info.num_classes
    ...
    write_json(args.config_path, config_record)
    ...
    print(f"Dataset: {args.dataset}, classes: {dataset_info.num_classes}, image_size: {args.image_size}")
