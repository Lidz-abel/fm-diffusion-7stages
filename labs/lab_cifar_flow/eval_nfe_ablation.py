...
def parse_args():
    ...
    parser.add_argument("--figure_path", type=str, default=str(ROOT / "figures" / "cifar_flow" / "nfe_ablation_unet.png"))
    parser.add_argument("--csv_path", type=str, default=str(ROOT / "results" / "cifar_flow" / "nfe_ablation.csv"))
...
def main():
    ...
    for nfe in args.nfe_list:
        ...
        summary = tensor_summary(samples)
        row = {
    ...
    save_labeled_image_rows(
        ...
        title=f"NFE Ablation | solver={args.solver}, cfg={args.cfg_scale}, samples/class={args.num_per_class}",
    )
    print(f"Saved NFE ablation figure to: {args.figure_path}")
    print(f"Saved NFE ablation CSV to: {args.csv_path}")
