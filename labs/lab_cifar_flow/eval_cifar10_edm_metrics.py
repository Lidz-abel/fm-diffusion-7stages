...
from labs.lab_cifar_flow.sample_cifar10_edm import load_model
...
def parse_args():
    ...
    parser.add_argument(
        "--save_grid",
        type=str,
        default=str(ROOT / "figures" / "cifar_flow" / "metrics_edm_preview.png"),
    )
    parser.add_argument(
        "--json_path",
        type=str,
        default=str(ROOT / "results" / "cifar_flow" / "metrics_edm.json"),
    )
    parser.add_argument(
        "--csv_path",
        type=str,
        default=str(ROOT / "results" / "cifar_flow" / "metrics_edm.csv"),
    )
...
def collect_fake_features(
    ...
    pbar = tqdm(total=args.num_samples, desc="Fake EDM sample features")
...
def main():
    ...
    real_stats = stats_to_device(compute_stats(real_features), device)
    fake_stats = stats_to_device(compute_stats(fake_features, fake_logits), device)
