...
def parse_args():
    ...
    parser.add_argument("--cfg_scales", type=float, nargs="+", default=[0, 1, 2, 4])
    parser.add_argument("--num_per_class", type=int, default=8)
    parser.add_argument("--device", type=str, default="cuda")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--figure_path", type=str, default=str(ROOT / "figures" / "cifar_flow" / "guidance_ablation_unet.png"))
    parser.add_argument("--csv_path", type=str, default=str(ROOT / "results" / "cifar_flow" / "guidance_ablation.csv"))
...
def main():
    ...
    for cfg_scale in args.cfg_scales:
        samples, timing = time_sampling(
            lambda cfg_scale=cfg_scale: sample_flow(
                model=model,
        ...
        row_labels.append(f"scale={cfg_scale:g}")
    ...
    save_labeled_image_rows(
        ...
        title=f"Guidance Ablation | solver={args.solver}, NFE={args.nfe}, samples/class={args.num_per_class}",
    )
    print(f"Saved guidance ablation figure to: {args.figure_path}")
    print(f"Saved guidance ablation CSV to: {args.csv_path}")
